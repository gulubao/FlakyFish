    ...
    def js_vars(self):
        return dict(
                   highcharts_series = [
                       {
                        'name' : 'stock_1_price',
                        'data' :  [round(g.stock_1_price,2) for g in self.player.in_all_rounds()]
                        }, {
                        'name' : 'stock_2_price',
                        'data' :  [round(g.stock_2_price,2) for g in self.player.in_all_rounds()]
                        }, {
                        'name' : 'stock_3_price',
                        'data' :  [round(g.stock_3_price,2) for g in self.player.in_all_rounds()]
                        }, {
                        'name' : 'stock_4_price',
                        'data' :  [round(g.stock_4_price,2) for g in self.player.in_all_rounds()]
                        }, {
                        'name' : 'stock_5_price',
                        'data' :  [round(g.stock_5_price,2) for g in self.player.in_all_rounds()]
                        }, {
                        'name' : 'stock_6_price',
                        'data' :  [round(g.stock_6_price,2) for g in self.player.in_all_rounds()]
                        }
                       ]
                    )
...
#%%
    ...
    def before_next_page(self):
        ...
        self.player.money = self.player.money + \
                            self.player.stock_1_sell_amount * self.player.stock_1_price + \
                            self.player.stock_2_sell_amount * self.player.stock_2_price + \
                            self.player.stock_3_sell_amount * self.player.stock_3_price + \
                            self.player.stock_4_sell_amount * self.player.stock_4_price + \
                            self.player.stock_5_sell_amount * self.player.stock_5_price + \
                            self.player.stock_6_sell_amount * self.player.stock_6_price
...
#%% 动态验证
    def error_message(self, values):
        ...
        if  values['stock_1_bid_amount'] * self.player.stock_1_price + \
            values['stock_2_bid_amount'] * self.player.stock_2_price + \
            values['stock_3_bid_amount'] * self.player.stock_3_price + \
            values['stock_4_bid_amount'] * self.player.stock_4_price + \
            values['stock_5_bid_amount'] * self.player.stock_5_price + \
            values['stock_6_bid_amount'] * self.player.stock_6_price \
            > self.player.money:
...
### 动态验证END
    ...
    def before_next_page(self):
        ...
        self.player.money = self.player.money - \
                    self.player.stock_1_bid_amount * self.player.stock_1_price - \
                    self.player.stock_2_bid_amount * self.player.stock_2_price - \
                    self.player.stock_3_bid_amount * self.player.stock_3_price - \
                    self.player.stock_4_bid_amount * self.player.stock_4_price - \
                    self.player.stock_5_bid_amount * self.player.stock_5_price - \
                    self.player.stock_6_bid_amount * self.player.stock_6_price


class Force_WaitPage(Page):
    def get_timeout_seconds(self):
        return self.player.force_waitpage_timeout_seconds()
...
class Clear(Page):
    ...
    def vars_for_template(self):
        self.player.final_money = self.player.money + \
                            self.player.stock_1_amount * self.player.stock_1_price + \
                            self.player.stock_2_amount * self.player.stock_2_price + \
                            self.player.stock_3_amount * self.player.stock_3_price + \
                            self.player.stock_4_amount * self.player.stock_4_price + \
                            self.player.stock_5_amount * self.player.stock_5_price + \
                            self.player.stock_6_amount * self.player.stock_6_price
        return {
            'round_number': self.round_number,
            'money': format(round(self.player.final_money, 2), ',')
        }
...
page_sequence = [ReadOnly, SellPage_sellable, BuyPage_buyable, Force_WaitPage, Clear]